...
import { createContactMessage, createExportInquiry, createProduct, createOrder, getProductBySlug, listAllOrders, listAllProducts, listExportInquiries, listOrdersForUser, listProducts } from "./db";
...
export const appRouter = router({
  ...
  admin: router({ products: adminProcedure.query(() => listAllProducts()), orders: adminProcedure.query(() => listAllOrders()), createProduct: adminProcedure.input(productInput).mutation(({ input }) => createProduct(input)) }),
