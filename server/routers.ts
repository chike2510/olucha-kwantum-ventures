...
import { createContactMessage, createExportInquiry, createProduct, updateProduct, createOrder, createBlogPost, updateBlogPost, getProductBySlug, listPublishedBlogPosts, getPublishedBlogPost, listAllBlogPosts, listAllOrders, listAllProducts, listExportInquiries, listOrdersForUser, listProducts, updateOrderStatus } from "./db";
...
const blogPostInput = z.object({ title: z.string().min(4).max(220), slug: z.string().regex(/^[a-z0-9-]+$/).max(240), excerpt: z.string().min(10), content: z.string().min(20), status: z.enum(["draft", "published"]).default("draft") });
...
export const appRouter = router({
  ...
  catalogue: router({ list: publicProcedure.input(z.object({ search: z.string().optional(), category: z.string().optional() }).optional()).query(({ input }) => listProducts(input?.search, input?.category)), bySlug: publicProcedure.input(z.object({ slug: z.string() })).query(({ input }) => getProductBySlug(input.slug)) }), news: router({ list: publicProcedure.query(() => listPublishedBlogPosts()), bySlug: publicProcedure.input(z.object({ slug: z.string() })).query(({ input }) => getPublishedBlogPost(input.slug)) }),
  ...
  admin: router({ products: adminProcedure.query(() => listAllProducts()), orders: adminProcedure.query(() => listAllOrders()), blogPosts: adminProcedure.query(() => listAllBlogPosts()), updateOrderStatus: adminProcedure.input(orderStatusInput).mutation(({ input }) => updateOrderStatus(input.id, input.status)), createBlogPost: adminProcedure.input(blogPostInput).mutation(({ input }) => createBlogPost({ ...input, publishedAt: input.status === "published" ? new Date() : null })), updateBlogPost: adminProcedure.input(blogPostUpdateInput).mutation(({ input }) => { const { id, ...values } = input; return updateBlogPost(id, { ...values, publishedAt: values.status === "published" ? new Date() : values.status === "draft" ? null : undefined }); }), createProduct: adminProcedure.input(productInput).mutation(({ input }) => createProduct(input)), updateProduct: adminProcedure.input(productUpdateInput).mutation(({ input }) => { const { id, ...values } = input; return updateProduct(id, values); }) }),
