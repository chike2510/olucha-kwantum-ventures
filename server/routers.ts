...
import { createContactMessage, createExportInquiry, createProduct, createOrder, getProductBySlug, listAllBlogPosts, listAllOrders, listAllProducts, listExportInquiries, listOrdersForUser, listProducts } from "./db";
...
export const appRouter = router({
  ...
  admin: router({ products: adminProcedure.query(() => listAllProducts()), orders: adminProcedure.query(() => listAllOrders()), blogPosts: adminProcedure.query(() => listAllBlogPosts()), createProduct: adminProcedure.input(productInput).mutation(({ input }) => createProduct(input)) }),
