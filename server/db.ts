...
export async function listProducts(search?: string, category?: string) { const db = await getDb(); if (!db) return []; const filters = [eq(products.isActive, 1)]; if (category && category !== "All products") filters.push(eq(products.category, category)); if (search) filters.push(or(like(products.name, `%${search}%`), like(products.category, `%${search}%`), like(products.description, `%${search}%`)) as never); return db.select().from(products).where(or(...filters) as never).orderBy(desc(products.createdAt)); }
export async function getProductBySlug(slug: string) { const db = await getDb(); if (!db) return undefined; const result = await db.select().from(products).where(eq(products.slug, slug)).limit(1); return result[0]; }
...
export async function createExportInquiry(input: typeof exportInquiries.$inferInsert) { const db = await getDb(); if (!db) throw new Error("Database unavailable"); const result = await db.insert(exportInquiries).values(input); return { id: result[0].insertId }; }
